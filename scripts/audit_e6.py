...
file_path = 'excels/6_asses.masses_E6Proxy.xlsx'
wb = openpyxl.load_workbook(file_path, data_only=True)

rules = ['Machine', 'Machines', 'Oom', 'Nonkel', 'Boerderij', 'ezel', 'hoe-hoe', 'U ', ' u ', 'Uw ', ' uw ', 'Muilegem', 'Ass Power', 'Praise be']
...
for sn in sheets_to_scan:
    ...
    for idx, row in enumerate(ws.iter_rows(values_only=True), 1):
        ...
        found_rule = False
        for r in rules:
            if r.lower() in nl.lower() or r.lower() in en.lower():
                found_rule = True
                break
