#!/usr/bin/env python3
"""E5 canon sweep scanner — flags candidates per sheet.

Adds to E4 scanner:
  §6.10  Acte/Actes/Acten → Nummer/Nummers
  §6.12  Nijg(t/en/de) → fel
  §10.1  doekjes rond winden → doekjes om winden
  §18    Wereldtournee (capped) → wereldtournee (retcon)
  §7.2   Circusdirecteur mid-sentence drift (lowercase unless title-before-name)

Carries forward all E4 patterns (§2 nie, §3.1 Muilebeek, §3.4 Mecha+exception,
§3.6 Boerderij, §6.1 Hudo/Sekreet/Privaat, §6.2 Oom (non-Smart Ass),
§6.16 Beroep/job lowercase, §7.1 Ezel cap, §8.1 Machien, §8.2 Machine
compounds, §4.3 retired monikers, §4.4 Schoon Beest, §5.4 Stop imperative,
§9.1 apostrophe, §9.2 mid-sentence Ik, §12.1 Kameraad, §14.1 retired slogans).

Also extracts speaker from column A (Key) rather than B (Description) for
more accurate §6.2 Oom check.
"""
import re
import sys
...
XLSX = Path('excels/5_asses.masses_E5Proxy.xlsx')
...
def scan_sheet(ws, sheet_name):
    ...
    for r, row in enumerate(ws.iter_rows(min_row=2, values_only=True), 2):
        ...
        # §7.2 Circusdirecteur mid-sentence cap drift
        for m in re.finditer(r"\bCircusdirecteur\b", nl):
            if m.start() == 0:
                continue
            prev = nl[max(0, m.start()-2):m.start()]
            if re.search(r"[.!?]\s$", prev + ' '):
                continue
            # If followed by a capitalized name token, it's a title — keep
            after = nl[m.end():m.end()+30].lstrip()
            if after and after[0].isupper():
                continue
            add('§7.2', 'Circusdirecteur mid-sentence — lowercase unless title before name')
            break
        ...
        # §18 retcon: capped Wereldtournee → wereldtournee (NEW for E5)
        if re.search(r"\bWereldtournee\b", nl):
            add('§18', 'Wereldtournee (cap) → wereldtournee (retcon)')
