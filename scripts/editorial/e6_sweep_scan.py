#!/usr/bin/env python3
"""E6 canon sweep scanner — flags candidates per sheet.

Adds to E5 scanner:
  §6.11  jansen (any case) → per-speaker fix (BattleHard Thirsty / BadCave Bad)
  §7.4   God/Goden speakers: lowercase u/uw → should be U/Uw (§5.2)
  §12.2  Sturdy motto detection (E6_Stable2F context)
  §7.2   Now flags lowercase circusdirecteur/tournee (retcon REVERSED — cap everywhere)
  §18    Now flags lowercase wereldtournee (retcon REVERSED — cap everywhere)

Carries forward all E5 patterns (§2 nie, §3.1 Muilebeek, §3.4 Mecha+exception,
§3.6 Boerderij, §6.1 Hudo/Sekreet/Privaat, §6.2 Oom (non-Smart Ass),
§6.10 Acte, §6.12 Nijg, §6.16 Beroep/job lowercase, §7.1 Ezel cap, §8.1 Machien,
§8.2 Machine compounds, §10.1 doekjes, §4.3 retired monikers, §4.4 Schoon Beest,
§5.4 Stop imperative, §9.1 apostrophe, §9.2 mid-sentence Ik, §12.1 Kameraad,
...
XLSX = Path('excels/6_asses.masses_E6Proxy.xlsx')
