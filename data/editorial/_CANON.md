...
- **E6_BadCave J55** (Bad Ass): ✅ pushed 2026-05-12 (§6.11+§2+§7.1 sweep).
...
- **Canonical frame:** `Naar de hel met die slechte, zielloze, werk-afpakkende, kind-dodende Machines.` (Tom 2026-05-12).
- **Adjective list:** `slechte, zielloze, werk-afpakkende, kind-dodende` — fixed order, fixed hyphens.
- **Exception:** `KWAADAARDIG!` (emphatic) stays.
- **Status:** ✅ E6_World J142 pushed 2026-05-12. E1_Stable2F J56 ✅ pushed 2026-05-12.
...
- **Decided:** 2026-05-10.
- **Status:** ✅ E6_World J160 pushed 2026-05-12.
...
| ~~Q19~~ | ~~Proper / Resentful / Bleak Ass register~~ — **RESOLVED 2026-05-11**: je/jij with formal-address `registerExceptions` (parallel to Sad/Nice/Slow). Names denote personality/social-positioning, not dialect. The observed `u/uw` cells = formal-context exception for these je/jij chars. **2026-05-12 ADDENDUM (Tom):** Resentful Ass also permits `gij` for cold/distant peer address (E7_Holding2 J18 kept as `Gij hebt geluk, gij hebt nog een deel van uw Kudde.`). Codex je/jij default + formal `u/uw` exception + Resentful's cold-peer `gij` exception — all three allowed for Resentful Ass per context. Register scanner must NOT flag Resentful Ass `gij` use. | §1 codex stubs ✅ |
...
| **E7** | ✅ **COMPLETE 2026-05-12** — Push 1: 8 sweep cells (Opening J5 job→Job §6.16; Opening J12 ezels→Ezels §7.1; Holding1 J8 nie→niet §2; ShippingTwo J5/J7/J9/J12 ezels→Ezels §7.1 — Factory PA, Andy, Derriere; BigBattle J10 ezeltje→Ezeltje §7.1 — Derriere diminutive). Push 2 (register retro): J18 Resentful Ass tested gij→je then **REVERTED** — Q19 addendum: Resentful Ass keeps cold-peer `gij`+`uw` exception. §8.2 Camion-Machine clean. §6.11 jansen extinct confirmed. Round-trip: 0 diffs. ⚠️ GAP: scanners lacked register check; extension + E0–E9 retro-scan pending (must exclude Resentful Ass `gij`). | ✅ Done |
...
| **E10** | ✅ **COMPLETE 2026-05-12** — Push 1: 26 cells (Hard J44 Hoeve §3.6; Government J6/J25/J145/J148/J163/J169/J172/J179/J182 + J23/J61/J65/J93/J97/J252/J269/J270 U/Uw §7.4; ProphetSpeech J6/J109/J111 + Credits J73/J74/J116 §7.1/§14.1; Epilogue J20/J33 Hoeve §3.6). Push 2: 3 drift cells (Government J76 Skinny je §5.0; J158 Gaunt je §5.0; J203 Resentful ge→je §5.0). Push 3: 3 translation-bug cells (J74 paste-error §EN-co-auth; J75 paraphrase §EN-co-auth; J170 duplicate §EN-co-auth). Tooling: `e10_alignment_scan.py` (length-ratio + cross-row similarity heuristic). Helpful Ass moved ge/gij locked (Q8 partial revision). Total: 32 cells. Round-trip: 0 diffs all pushes. | ✅ Done |
...
- **2026-05-12 | Sturdy Ass / E9_BadCave J43** | Motto-fragment line uses 3-adj form (`slechte, zielloze, kind-dodende`) per 3-adj EN (`evil, soul-less, child-killing`). Sentence is NOT the canonical `Naar de hel met die…` frame from §12.2; deferred to EN co-auth. Flag for future Tom override if 4-adj canon should propagate to all Sturdy motto-bearing lines regardless of EN length.
- **2026-05-12 | Cole-Machine / E9_BadCave J63** | ✅ RESOLVED. `Stevige. Stop.` → `Stevige. Stopt.` pushed in E9 Push 2 (tail).
- **2026-05-12 | Cole-Machine / E9_BadCave J64** | ✅ RESOLVED. `Probeer de machine…` → `Probeert de Machine…` pushed in E9 Push 2 (tail).
- **2026-05-12 | Cole-Machine §5.4 corpus sweep / E9** | ✅ RESOLVED — broader scan across ge/gij speakers caught J34 (Geef→Geeft), J81 (Luister→Luistert), J135 GoldenAss (Ga→Gaat) in addition to J63/J64. 5 cells total pushed in E9 Push 2.
- **2026-05-12 | scanner gap** | `e9_sweep_scan.py` §5.4 rule only matched `Stop` imperative; missed other ge/gij imperatives. **For E10 scanner: expand regex to flag any sentence-start bare-stem imperative from ge/gij speakers; verb stems already ending in -t (Wacht, Laat) are clean no-ops.**
