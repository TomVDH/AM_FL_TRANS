...
| **E9** | ✅ **COMPLETE 2026-05-12** — Push 1: 8 cells (GoldenAss J10 Ezel §7.1; GoldenAss J19/J47/J83/J130 God-register U/Uw §7.4; BadCave J9/J14/J24 Ezel(s) §7.1). Verify-only: BadCave J43 Sturdy motto retained EN-co-authoritative 3-adj form; J68 + MineEscape J18–21 already canonical. Round-trip: 0 diffs. | ✅ Done |
...
- **2026-05-12 | Cole-Machine / E9_BadCave J63** | `Stevige. Stop.` — Cole-Machine is ge/gij (§5.0); imperative should be `Stopt.` per §5.4. Drift surfaced in E9 sweep but mis-skipped on first pass (register not checked); follow-up push staged.
- **2026-05-12 | Cole-Machine / E9_BadCave J64** | `Probeer de machine niet kapot te maken.` — TWO drifts: (1) `Probeer` ge/gij imperative → `Probeert` per §5.4 (same pattern as J63); (2) `machine` lowercase → `Machine` per §7.3 (game-systemic race name cap). Not flagged by current scanner (regex only catches `Stop`). Carry-over for E10 handoff or dedicated §5.4 corpus sweep.
- **2026-05-12 | scanner gap** | `e9_sweep_scan.py` §5.4 rule only matches `Stop` imperative; misses other ge/gij imperatives (`Probeer`, `Zeg`, `Sla`, `Doe`, etc.). E10 scanner should expand the regex to flag any sentence-start bare-stem imperative from ge/gij speakers.
