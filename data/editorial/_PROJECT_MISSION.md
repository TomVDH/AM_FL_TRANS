...
| **Routine sync ops are part of approved action (codified 2026-04-27)** | When user has authorized a sync (commit + push to a branch they just told you to update), execute directly. Do NOT: open multi-option menus for tangential choices (amending commit author, switching remote URL, alternate identity flows); repeat "want me to push?" after they've already said yes; ladder pre-flight options for one-line `git push`. Reversible repo-local fixes that enable the push (e.g., switching origin to SSH when SSH keys exist) are in-scope without re-asking. **Verbatim from Tom**: *"I JUST need your findings remote so we can take over there. it's not that hard nor deep."* |
| **Trust your own actions (codified 2026-04-28)** | When user asks about a step you just executed (e.g., "did the apply land?"), you should KNOW from your own tool output. Don't re-verify what you already saw. Confidence in own work matters. **Verbatim from Tom**: *"I mean you'd KNOW if you ran that tool, wouldn't you."* |
| **No destructive ops without explicit approval (codified 2026-04-28)** | Don't propose corpus-wide scans, batch fixes, or other potentially-destructive operations as casual offers when the user has already given a focused directive. **Verbatim from Tom**: *"I cannot have you do potentially destructive shit, come on."* |
