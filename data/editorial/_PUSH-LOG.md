...
## 🎯 E1 Push 3 — deep-eyeball DRIFT batch (17 cells across 5 sub-tabs)

Sourced from `data/editorial/audit-2026-05-12-deep-eyeball.md` (22 E1 cells flagged DRIFT) → live API read via _api_throttle → Tom sign-off 2026-05-13. 17 applied, 3 explicitly kept (J14 FarmHouseInt, J13 RedFields, J11 Stable1F), 2 already-applied (J67/J69 TheProtest from slogan retcon).

Override: J7 FarmHouseInt — Tom full rewrite preserving "hustle and bustle" parallel.

### E1_FarmHouseInt_localization (2 cells)

| Sheet | Cell | Before | After | Rule |
|---|---|---|---|---|
| E1_FarmHouseInt_localization | J5 | `Vertel ons eens... wat is het het geheim van je succes?` | `Vertel ons eens... wat is het geheim van je succes?` | typo (duplicate `het het`) |
| E1_FarmHouseInt_localization | J7 | `We moeten bereid zijn hard te werken, en dat aan een product dat levens verandert.` | ` We moeten bereid zijn hard te werken, en dat terwijl we een product hebben dat levens verandert.` | Tom override (parallel-structure rewrite) |

### E1_Farm_localization (6 cells)

| Sheet | Cell | Before | After | Rule |
|---|---|---|---|---|
| E1_Farm_localization | J17 | `…De mensen moeten ergens in naar het dorp zijn.` | `…De Mensen moeten ergens in het dorp zijn.` | §7.3 + Dutch grammar (broken `in naar`) |
| E1_Farm_localization | J39 | `…voor het p-p-protest.` | `…voor het P-P-Protest.` | §7.3 (game-system cap, stutter preserved) |
| E1_Farm_localization | J59 | `…het protest van Oude Ezel…` | `…het Protest van Oude Ezel…` | §7.3 |
| E1_Farm_localization | J60 | `…zal de ziel van wijlen Luie Ezel…` | `…zal de Ziel van wijlen Luie Ezel…` | §7.3 (game-system Ziel) |
| E1_Farm_localization | J94 | `Maar ge kunt toch verdomme toch niet zomaar jullie jobs terug eisen.` | `Maar ge kunt verdomme toch niet zomaar jullie Jobs terug eisen.` | dup `toch` + §6.16 game-system Jobs |
| E1_Farm_localization | J102 | `Jullie idee van richting en eigenwaarde…` | `Jullie idee van waarde en eigenwaarde…` | §13 mistranslation (value → waarde) |

### E1_Stable1F_localization (2 cells)

| Sheet | Cell | Before | After | Rule |
|---|---|---|---|---|
| E1_Stable1F_localization | J8 | `…dat we de Mensen er aan te herinneren…` | `…dat we de Mensen eraan herinneren…` | Dutch grammar (dat-clause needs finite verb, not te-infinitive) |
| E1_Stable1F_localization | J10 | `…onze alleréérste PROTEST.` | `…onze allereerste PROTEST.` | Dutch spelling (non-standard accent) |

### E1_Stable2F_localization (1 cell)

| Sheet | Cell | Before | After | Rule |
|---|---|---|---|---|
| E1_Stable2F_localization | J22 | `De mensen... ze zijn me komen pakken in 't donker...` | `De Mensen... ze zijn me komen pakken in 't donker...` | §7.3 |

### E1_TheProtest_localization (6 cells)

| Sheet | Cell | Before | After | Rule |
|---|---|---|---|---|
| E1_TheProtest_localization | J21 | `…zouden de mensen dat echt doen?` | `…zouden de Mensen dat echt doen?` | §7.3 |
| E1_TheProtest_localization | J22 | `…dat de mensen mogelijk een foutje gemaakt hebben.` | `…dat de Mensen mogelijk een foutje gemaakt hebben.` | §7.3 (MENU.Dialog UI string) |
| E1_TheProtest_localization | J23 | `…dat de mensen zijn verdorven door de Machines.` | `…dat de Mensen zijn verdorven door de Machines.` | §7.3 |
| E1_TheProtest_localization | J39 | `…jij bent een luisteraar, niet een leider.` | `…jij bent een luisteraar, geen leider.` | Dutch idiom (niet een → geen) |
| E1_TheProtest_localization | J61 | `Ezels zijn altijd al de ruggegraat van deze Hoeve geweest…` | `Ezels zijn altijd al de ruggengraat van deze Hoeve geweest…` | Dutch spelling (ruggegraat → ruggengraat) |
| E1_TheProtest_localization | J147 | `…heeft gëinspireerd.` | `…heeft geïnspireerd.` | Dutch spelling (diaeresis on i, not ë) |

### Verify-only (no edit per Tom 2026-05-13)
- `E1_FarmHouseInt_localization` J14 — `iedereen zullen halen` agreement quirk kept (Tom decision, Mme. Derrière voice).
- `E1_RedFields_localization` J13 — `Feel The Love!` English-bleed kept (Tom decision, signature welcome-ritual flavor).
- `E1_Stable1F_localization` J11 — `een goed moraal` adj-gender kept (Tom decision).
- `E1_Farm_localization` J29 — Schoon Beest §4.4 preserve (push-confirmed earlier batch).

### Tooling
- `scripts/editorial/propose-fixes-e1.py` — live-API read + transform preview (uses _api_throttle).
- `scripts/editorial/apply-fixes-e1.py` — safe local writer.
