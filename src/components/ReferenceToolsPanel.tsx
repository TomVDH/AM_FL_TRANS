...
const ReferenceToolsPanel: React.FC<ReferenceToolsPanelProps> = ({
  ...
  const [expandedCodexEntry, setExpandedCodexEntry] = useState<number | null>(null);
          ...
          <div className="border border-gray-200 dark:border-gray-600 overflow-hidden max-h-72 overflow-y-auto custom-scrollbar" style={{ borderRadius: '3px' }}>
            {isLoadingCodex ? (
              ...
              filteredCodexData.map((entry, idx) => {
                const hasInfo = entry.category === 'CHARACTER' && (entry.bio || entry.gender || entry.dialogueStyle);
                  ...
                  <div key={idx}>
                    <div
                      className={`group flex items-center justify-between px-2.5 py-2 transition-colors hover:bg-gray-50 dark:hover:bg-gray-700 ${
                        idx !== 0 ? 'border-t border-gray-100 dark:border-gray-700' : ''
                      } ${hasInfo ? 'cursor-pointer' : ''} ${isExpanded ? 'bg-purple-50 dark:bg-purple-900/20' : ''}`}
                      onClick={() => {
                        if (hasInfo) {
                          setExpandedCodexEntry(isExpanded ? null : idx);
                        }
                      }}
                    >
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        ...
                        {hasInfo ? (
                          <svg className={`shrink-0 w-3 h-3 text-purple-400 dark:text-purple-500 transition-transform duration-200 ${isExpanded ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {/* Gender badge inline */}
                        {entry.gender && (
                          <span className="text-[9px] text-purple-500 dark:text-purple-400 shrink-0">
                            {entry.gender === 'male' ? '\u2642' : '\u2640'}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={(e) => { e.stopPropagation(); insertCharacterName(entry.dutch); }}
                          className="p-1 bg-gray-100 dark:bg-gray-600 hover:bg-gray-200 dark:hover:bg-gray-500 text-gray-600 dark:text-gray-300 transition-colors"
                          style={{ borderRadius: '2px' }}
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); copyToClipboard(entry.dutch); }}
                          className="p-1 bg-gray-100 dark:bg-gray-600 hover:bg-gray-200 dark:hover:bg-gray-500 text-gray-600 dark:text-gray-300 transition-colors"
                          style={{ borderRadius: '2px' }}
                          title="Copy to clipboard"
                        >
                    ...
                    {isExpanded && hasInfo && (
                      <div className="px-3 py-2 bg-purple-50 dark:bg-purple-900/20 border-t border-purple-100 dark:border-purple-800 space-y-2">
                        {/* Gender */}
                        {entry.gender && (
                          <div className="flex items-center gap-1.5">
                            <span className="text-[9px] uppercase tracking-wider text-purple-500 dark:text-purple-400 font-bold">Gender</span>
                            <span className="text-[11px] px-1.5 py-0.5 bg-purple-200 dark:bg-purple-700 text-purple-700 dark:text-purple-200 font-medium" style={{ borderRadius: '2px' }}>
                        ...
                        {entry.dialogueStyle && (
                          <div>
                            <div className="text-[9px] uppercase tracking-wider text-purple-500 dark:text-purple-400 font-bold mb-0.5">Style</div>
                        ...
                        {entry.bio && (
                          <div>
                            <div className="text-[9px] uppercase tracking-wider text-purple-500 dark:text-purple-400 font-bold mb-0.5">Bio</div>
