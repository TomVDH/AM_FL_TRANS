...
const ReferenceToolsPanel: React.FC<ReferenceToolsPanelProps> = ({
  ...
  const handleTabChange = (tab: TabType) => {
    setActiveTab(tab);
    // Enable global search when entering search tab to search across all files
    if (tab === 'search') {
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">File</span>
              <select
                value={selectedXlsxFile}
                onChange={(e) => loadXlsxData(e.target.value)}
                className="px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:border-gray-500 focus:ring-1 focus:ring-gray-500/20"
            ...
            {/* Sheet selector - only when not global */}
            {xlsxData && !globalSearch && (
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Sheet</span>
                <select
            ...
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input
                ...
                className="w-3.5 h-3.5 rounded border-gray-300 text-gray-700 focus:ring-gray-500 focus:ring-offset-0"
                disabled={isLoadingXlsx}
              />
              <span className="text-[10px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">All sheets</span>
            </label>
          </div>
              ...
              getFilteredEntries().map((entry: any, idx: number) => (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    {/* Row number badge */}
                    <span className="shrink-0 text-[9px] font-mono text-gray-400 dark:text-gray-500 w-6 text-right">
                      {entry.row}
                    </span>
                    ...
                    <span className="text-xs font-medium text-gray-900 dark:text-gray-100 truncate">
                      {entry.translatedDutch || '(no translation)'}
                    </span>

                    {/* Sheet name badge (if global search) */}
                    {globalSearch && entry.sheetName && (
                      <span className="shrink-0 text-[9px] px-1 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400" style={{ borderRadius: '2px' }}>
                        {entry.sheetName.replace(/^E\d+_/, '').replace(/_localization$/i, '').slice(0, 12)}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {entry.translatedDutch && (
                      <button
                        ...
                        style={{ borderRadius: '2px' }}
                        title={`Insert "${entry.translatedDutch}"`}
                      >
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
